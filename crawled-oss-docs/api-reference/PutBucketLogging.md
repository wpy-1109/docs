...
The oss:PutBucketLogging permission is required for enabling logging for a bucket by calling the PutBucketLogging operation. For more information, see Attach a custom policy to a RAM user.
...
This parameter is required when you enable logging.
...
Child nodes: TargetBucket, TargetPrefix and LoggingRole
...
The role with permissions for logging.
...
All headers in the response to a PutBucketLogging request are common response headers. For more information, see Common response headers.
...
For information about the ossutil command that corresponds to the PutBucketLogging operation, see put-bucket-logging.
...
The error message returned because the source bucket does not exist.
...
The error message returned because the source bucket and the destination bucket do not belong to the same data center.
...
The error message returned because the Content-MD5 value of the message body is inconsistent with the Content-MD5 value of the request header.
...
The error message returned because the XML format in the request is invalid.
...
The error message returned because the requester is not the owner of the destination bucket.
...
The error message returned because the requester is not the owner of the source bucket.
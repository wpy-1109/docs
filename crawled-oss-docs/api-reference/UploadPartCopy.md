...
You must use UploadPartCopy to copy a file larger than 1 GB. To copy a file smaller than 1 GB in a single operation, see CopyObject.
...
The source object to copy. You must have read permissions on this object.
...
The range of the source object to copy. For example, if you set this header to bytes=0-9, the first 10 bytes of the object are copied.

Default value: None

If you do not specify this header, the entire source object is copied.

If you specify this header, the response includes the total length of the file and the range of the current copy. For example, `Content-Range: bytes 0-9/44` indicates that the total file length is 44 bytes and the copied range is from byte 0 to byte 9.

If the specified range is invalid, the entire source object is copied, and the response does not include Content-Range.
...
The copy operation is performed only if the ETag of the source object matches the specified ETag. Otherwise, the 412 Precondition Failed error is returned.
...
If the provided ETag value does not match the Object's ETag, the file is transferred and a 200 OK response is returned. Otherwise, a 304 Not Modified response is returned.
...
The file is copied and 200 OK is returned only if the time specified in this header is the same as or later than the actual modification time of the file. Otherwise, the 412 Precondition Failed error is returned.
...
The file is copied and 200 OK is returned only if the specified time is earlier than the actual modification time of the file. Otherwise, 304 Not Modified is returned.
...
For the ossutil command that corresponds to the UploadPartCopy operation, see upload-part-copy.
...
The UploadPartCopy operation is called for a bucket of the Archive storage class.
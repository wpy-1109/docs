...
The status of transfer acceleration Valid values:

true: indicates that transfer acceleration is enabled for the bucket.

false: indicates that transfer acceleration is disabled for the bucket.

For more information about other common response headers, such as x-oss-request-id and Date, contained in the response to a GetBucketTransferAcceleration request, see Common response headers.
...
For information about the ossutil command that corresponds to the GetBucketTransferAcceleration operation, see get-bucket-transfer-acceleration.
...
The error message returned because transfer acceleration is not configured for the bucket.
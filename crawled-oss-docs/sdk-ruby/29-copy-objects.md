# 使用Ruby SDK在同一地域的Bucket间拷贝文件

Source: https://help.aliyun.com/zh/oss/developer-reference/copy-objects

---

- 使用Ruby SDK在同一地域的Bucket间拷贝文件-对象存储-阿里云
...
使用`bucket.copy_object`拷贝一个文件，并通过`:meta_directive`参数指定文件元数据。拷贝时对文件元数据的处理有以下两种方式：
- 如果没有指定meta参数，则拷贝后的文件元数据与源文件相同。
- 如果指定了meta参数，则拷贝后的文件将使用新的元数据覆盖源文件的元数据。
...
[上一篇：列举文件（Ruby SDK）](/zh/oss/developer-reference/list-objects)[下一篇：删除文件（Ruby SDK）](/zh/oss/developer-reference/delete-objects)该文章对您有帮助吗？反馈

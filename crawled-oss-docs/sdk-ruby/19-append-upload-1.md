# 使用Ruby SDK向Object追加上传内容

Source: https://help.aliyun.com/zh/oss/developer-reference/append-upload-1

---

- 使用Ruby SDK向Object追加上传内容-对象存储-阿里云
...
- 当文件不存在时，调用`Bucket#append_object`接口会创建一个追加类型文件。
...
next_pos = bucket.append_object('my-object', 0) do |stream|
  100.times { |i| stream  'local-file-1')
...
[上一篇：断点续传上传（Ruby SDK）](/zh/oss/developer-reference/resumable-download-1)[下一篇：上传回调（Ruby SDK）](/zh/oss/developer-reference/upload-callbacks-1)该文章对您有帮助吗？反馈

# 使用OSS C# SDK V2判断对象文件是否存在

Source: https://help.aliyun.com/zh/oss/developer-reference/determine-whether-an-object-exists-using-oss-sdk-for-csharp-v2

---

- 使用OSS C# SDK V2判断对象文件是否存在-对象存储-阿里云
...
// 调用IsObjectExistAsync方法，判断目标对象文件是否存在。
var result = await client.IsObjectExistAsync(bucket, key);

// 输出操作完成信息
Console.WriteLine("IsObjectExist done");
// 输出检查结果（true表示对象存在，false表示不存在）
Console.WriteLine($"result: {result}");
```

## 相关文档

关于判断文件是否存在的完整示例代码，请参见[IsObjectExist.cs](https://github.com/aliyun/alibabacloud-oss-csharp-sdk-v2/blob/master/sample/IsObjectExist/Program.cs)。

[上一篇：管理文件（C# SDK V2）](/zh/oss/developer-reference/manage-files-using-oss-sdk-for-c-v2/)[下一篇：列举文件（C# SDK V2）](/zh/oss/developer-reference/list-objects-using-oss-sdk-for-csharp-v2)该文章对您有帮助吗？反馈

# 使用C# SDK永久和临时删除版本控制下的文件

Source: https://help.aliyun.com/zh/oss/developer-reference/delete-objects-8

---

- 使用C# SDK永久和临时删除版本控制下的文件-对象存储-阿里云
...
- 永久删除 以下代码用于指定versionId对Object进行永久删除：
```
using Aliyun.OSS;
...
```
- 临时删除 以下代码用于不指定versionId对Object进行临时删除：
```
using Aliyun.OSS;
using Aliyun.OSS.Common;
...
- 永久删除 以下代码用于指定versionId对多个Object及删除标记进行永久删除：
```
using Aliyun.OSS;
using Aliyun.OSS.Common;
// yourEndpoint填写Bucket所在地域对应的Endpoint。以华东1（杭州）为例，Endpoint填写为https://oss-cn-hangzhou.aliyuncs.com。
...
```
- 临时删除以下代码用于不指定versionId对多个Object进行临时删除。临时删除后，支持恢复该版本的Object。
```
using Aliyun.OSS;
using Aliyun.OSS.Common;
...
[上一篇：拷贝文件（C# SDK V1）](/zh/oss/developer-reference/copy-objects-in-version-bucket-using-oss-sdk-for-csharp-v1)[下一篇：解冻文件（C# SDK V1）](/zh/oss/developer-reference/restore-objects-6)该文章对您有帮助吗？反馈

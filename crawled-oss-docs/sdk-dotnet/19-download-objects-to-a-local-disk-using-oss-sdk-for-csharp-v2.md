# 使用C# SDK V2下载OSS文件到本地磁盘

Source: https://help.aliyun.com/zh/oss/developer-reference/download-objects-to-a-local-disk-using-oss-sdk-for-csharp-v2

---

- 使用C# SDK V2下载OSS文件到本地磁盘-对象存储-阿里云
...
本文介绍如何通过简单下载方法将存储空间（Bucket）中的文件（Object）下载到本地，此方法操作简便，适合快速将云端存储的文件下载到本地磁盘。
...
var key = "your object name";  // 必须项，指定下载的对象名称。格式（folder/objectName）
var filePath = "your file path";  // 必须项，指定下载到本地磁盘的文件路径
...
// 调用GetObjectToFileAsync方法，直接将OSS对象流式下载到本地磁盘中，避免大文件占用过多内存
...
// 打印上传结果
Console.WriteLine("GetObjectToFile done");  // 提示操作完成
...
关于下载文件到本地磁盘中的完整示例代码，请参见[GetObjectToFile.cs](https://github.com/aliyun/alibabacloud-oss-csharp-sdk-v2/blob/master/sample/GetObjectToFile/Program.cs)。

[上一篇：下载文件到内存（C# SDK V2）](/zh/oss/developer-reference/download-objects-to-the-local-memory-using-oss-sdk-for-csharp-v2)[下一篇：使用预签名URL下载（C# SDK V2）](/zh/oss/developer-reference/download-objects-using-presigned-urls-generated-with-oss-sdk-for-csharp-v2)该文章对您有帮助吗？反馈

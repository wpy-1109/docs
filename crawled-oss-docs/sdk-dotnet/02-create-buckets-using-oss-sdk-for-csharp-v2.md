...
Source: https://help.aliyun.com/zh/oss/developer-reference/create-buckets-using-oss-sdk-for-csharp-v2

---

- 使用C# SDK V2创建存储空间-对象存储-阿里云
...
存储空间（Bucket）是对象（Object）的容器，您上传的文件都将以对象的形式放在存储空间中。本文介绍如何通过C# SDK V2创建存储空间。
...
var bucket = "your bucket name";  // 必须项，设置创建的Bucket名称
...
// 调用PutBucketAsync方法创建新的存储桶，存储类型为标准存储
...
// 打印创建结果
Console.WriteLine("PutBucket done");  // 提示操作完成
...
关于创建存储空间的完整示例代码，请参见[PutBucket.cs](https://github.com/aliyun/alibabacloud-oss-csharp-sdk-v2/blob/master/sample/PutBucket/Program.cs)。

[上一篇：存储空间（C# SDK V2）](/zh/oss/developer-reference/manage-buckets-using-oss-sdk-for-c-2-0/)[下一篇：列举存储空间（C# SDK V2）](/zh/oss/developer-reference/list-buckets-using-oss-sdk-for-csharp-v2)该文章对您有帮助吗？反馈

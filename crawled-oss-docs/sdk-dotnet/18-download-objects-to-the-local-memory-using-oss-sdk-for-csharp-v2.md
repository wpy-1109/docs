# 使用C# SDK V2将OSS文件下载到内存

Source: https://help.aliyun.com/zh/oss/developer-reference/download-objects-to-the-local-memory-using-oss-sdk-for-csharp-v2

---

- 使用C# SDK V2将OSS文件下载到内存-对象存储-阿里云
...
本文介绍如何通过简单下载方法将存储空间（Bucket）中的文件（Object）下载到本地内存，此方法操作简便，适合快速将云端存储的文件下载到本地内存中。
...
var key = "your object key"; // 必须项，指定下载的对象名称。格式（folder/objectName）
...
//调用GetObjectAsync方法，默认使用流式模式获取对象，此时仅下载响应头，内容以流的形式返回
...
// 打印结果信息 
Console.WriteLine("GetObject done");  // 提示操作完成
...
关于简单下载文件到本地内存中的完整示例代码，请参见[GetObject.cs](https://github.com/aliyun/alibabacloud-oss-csharp-sdk-v2/blob/master/sample/GetObject/Program.cs)。

[上一篇：下载文件（C# SDK V2）](/zh/oss/developer-reference/download-file-using-oss-sdk-for-c-v2/)[下一篇：下载文件到本地（C# SDK V2）](/zh/oss/developer-reference/download-objects-to-a-local-disk-using-oss-sdk-for-csharp-v2)该文章对您有帮助吗？反馈

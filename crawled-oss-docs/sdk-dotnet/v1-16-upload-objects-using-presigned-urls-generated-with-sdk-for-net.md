# 使用C# SDK生成用于上传文件的预签名URL

Source: https://help.aliyun.com/zh/oss/developer-reference/upload-objects-using-presigned-urls-generated-with-sdk-for-net

---

- 使用C# SDK生成用于上传文件的预签名URL-对象存储-阿里云
...
默认情况下，OSS Bucket中的文件是私有的，仅文件拥有者拥有访问权限。您可以使用OSS C# SDK生成预签名URL，以允许他人通过该URL上传文件。在生成预签名URL时，可以自定义其过期时间以限制访问持续时长。在预签名URL有效期内，该URL可被多次访问。如果多次执行上传操作，会有文件覆盖的风险。超出有效期后，将无法进行上传，此时需要重新生成预签名URL。
...
- 本文以从环境变量读取访问凭证为例。如何配置访问凭证，请参见[配置访问凭证（C# SDK V1）](https://help.aliyun.com/zh/oss/developer-reference/configure-access-credentials-using-oss-sdk-for-csharp-v1)。
...
- 通过以下示例生成的预签名URL中如果包含特殊符号`+`，可能出现无法正常访问该签名URL的现象。如需正常访问该预签名URL，请将预签名URL中的`+`替换为`%2B`。
...
[上一篇：权限控制（C# SDK V1）](/zh/oss/developer-reference/manage-acls-using-oss-sdk-for-csharp-v1/)[下一篇：管理存储空间读写权限（C# SDK V1）](/zh/oss/developer-reference/manage-bucket-acls-3)该文章对您有帮助吗？反馈

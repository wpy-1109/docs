# 使用Android SDK配置基于Referer的防盗链规则

Source: https://help.aliyun.com/zh/oss/developer-reference/android-hotlink-protection

---

- 使用Android SDK配置基于Referer的防盗链规则-对象存储-阿里云
...
- 本文以OSS域名新建OSSClient为例。如果您希望通过自定义域名、STS等方式新建OSSClient，请参见[初始化（Android SDK）](https://help.aliyun.com/zh/oss/developer-reference/initialization-1#concept-32044-zh)。
...
OSSAsyncTask task = oss.asyncGetBucketReferer(request, new OSSCompletedCallback() {
    @Override
    public void onSuccess(GetBucketRefererRequest request, GetBucketRefererResult result) {
        // 获取存储空间Referer白名单列表。
        ArrayList list = result.getReferers();
        for (String ref : list){
...
## 相关文档
- 关于防盗链的完整示例代码，请参见[GitHub](https://github.com/aliyun/aliyun-oss-android-sdk/blob/master/oss-android-sdk/src/androidTest/java/com/alibaba/sdk/android/OSSBucketTest.java)。
...
[上一篇：数据安全（Android SDK）](/zh/oss/developer-reference/data-security-6/)[下一篇：数据校验（Android SDK）](/zh/oss/developer-reference/data-verification-3)该文章对您有帮助吗？反馈

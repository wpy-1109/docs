# 使用AndroidSDK按指定条件列举存储空间

Source: https://help.aliyun.com/zh/oss/developer-reference/list-buckets-4

---

- 使用AndroidSDK按指定条件列举存储空间-对象存储-阿里云
...
ossClient.asyncListBuckets(request, new OSSCompletedCallback() {
    ...
    public void onSuccess(ListBucketsRequest request, ListBucketsResult result) {
        ...
        for (int i = 0; i 
## 相关文档
- 关于列举存储空间的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/aliyun-oss-android-sdk/blob/master/oss-android-sdk/src/androidTest/java/com/alibaba/sdk/android/OSSBucketTest.java)。
...
[上一篇：创建存储空间（Android SDK）](/zh/oss/developer-reference/create-buckets-5)[下一篇：获取存储空间的访问权限（Android SDK）](/zh/oss/developer-reference/query-bucket-acls)该文章对您有帮助吗？反馈

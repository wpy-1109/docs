# 使用C++ SDK进行MD5和CRC64数据校验

Source: https://help.aliyun.com/zh/oss/developer-reference/data-verification-1

---

- 使用C++ SDK进行MD5和CRC64数据校验-对象存储-阿里云
...
说明 - putObject、getObject、appendObject、uploadPart支持CRC64校验。上传时默认开启CRC校验，如果客户端计算的CRC值与服务端返回的CRC值不一致，则会返回错误。
- 范围下载不支持CRC64校验。
...
[上一篇：客户端加密（C++ SDK）](/zh/oss/developer-reference/client-side-encryption-3)[下一篇：数据管理（C++ SDK）](/zh/oss/developer-reference/data-management-5/)该文章对您有帮助吗？反馈

# 使用C++SDK为存储空间配置静态网站托管

Source: https://help.aliyun.com/zh/oss/developer-reference/static-website-hosting-9

---

- 使用C++SDK为存储空间配置静态网站托管-对象存储-阿里云
...
- 要设置静态网站托管，您必须有`oss:PutBucketWebsite`权限；要获取静态网站托管，您必须有`oss:GetBucketWebsite`；要删除静态网站托管，您必须有`oss:DeleteBucketWebsite`权限。具体操作，请参见[为RAM用户授予自定义的权限策略](https://help.aliyun.com/zh/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip)。
...
int main(void)
{
    ...
    if (!outcome.isSuccess()) {
        ...
        std::cout 
## 相关文档
- 关于静态网站托管的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/aliyun-oss-cpp-sdk/blob/master/sample/src/bucket/BucketSample.cc)。
...
- 关于查看静态网站托管配置的API接口说明，请参见[GetBucketWebsite](https://help.aliyun.com/zh/oss/developer-reference/getbucketwebsite#reference-wvy-s4w-tdb)。
- 关于删除静态网站托管配置的API接口说明，请参见[DeleteBucketWebsite](https://help.aliyun.com/zh/oss/developer-reference/deletebucketwebsite#reference-zrl-msw-tdb)。

[上一篇：存储空间清单（C++ SDK）](/zh/oss/developer-reference/bucket-inventory-4)[下一篇：日志转存（C++ SDK）](/zh/oss/developer-reference/logging-10)该文章对您有帮助吗？反馈

# 使用Browser.js SDK管理文件元数据

Source: https://help.aliyun.com/zh/oss/developer-reference/manage-object-metadata-3

---

- 使用Browser.js SDK管理文件元数据-对象存储-阿里云
...
以下代码用于上传文件时指定元数据，并在文件上传后更新和查看文件元数据。
  ...
  查看元数据
    ...
    upload.addEventListener('click', () => {
     client.put(fileName, file, {
        // 指定自定义元数据。单个文件可以有多个元数据，但总大小不能超过8 KB。
        meta: {
          year: 2020,
          people: 'eliot'
        }
    ...
    check.addEventListener('click', () => {
      // 根据以上代码配置的元数据，您需要在Bucket跨域规则的暴露Headers配置项中添加x-oss-meta-year，否则console打印的meta为null。
...
## 相关文档
- 关于管理文件元数据的完整示例代码，请参见[GitHub示例](https://github.com/ali-sdk/ali-oss#putmetaname-meta-options)。
- 关于上传文件时设置或修改文件元数据的API接口说明，请参见[PutObject](https://help.aliyun.com/zh/oss/developer-reference/putobject#reference-l5p-ftw-tdb)。
- 关于查看文件元数据的API接口说明，请参见[HeadObject](https://help.aliyun.com/zh/oss/developer-reference/headobject#reference-bgh-cbw-wdb)。

[上一篇：解冻文件（Browser.js SDK）](/zh/oss/developer-reference/restore-objects-13)[下一篇：管理软链接（Browser.js SDK）](/zh/oss/developer-reference/manage-symbolic-links-17)该文章对您有帮助吗？反馈

# Browser.js SDK通过拷贝删除文件实现重命名

Source: https://help.aliyun.com/zh/oss/developer-reference/rename-an-object

---

- Browser.js SDK通过拷贝删除文件实现重命名-对象存储-阿里云
...
```
  ...
  拷贝
    ...
    client.copy('destobject.txt', 'srcobject.txt').then(r => {
    if (r.res.status === 200) {
    // 删除srcobject.txt。
    client.delete('srcobject.txt').then(j => {
      console.log(j);
    });
  }
});
...
关于重命名文件涉及的API接口说明，请分别参见[CopyObject](https://help.aliyun.com/zh/oss/developer-reference/copyobject#reference-mvx-xxc-5db)和[DeleteObject](https://help.aliyun.com/zh/oss/developer-reference/deleteobject#reference-iqc-mqv-wdb)。

[上一篇：列举文件（Browser.js SDK）](/zh/oss/developer-reference/list-objects-16)[下一篇：拷贝文件（Browser.js SDK）](/zh/oss/developer-reference/copy-objects-15)该文章对您有帮助吗？反馈

# 通过Browser.js SDK追加上传文件内容

Source: https://help.aliyun.com/zh/oss/developer-reference/append-upload-3

---

- 通过Browser.js SDK追加上传文件内容-对象存储-阿里云
...
## 注意事项
- 通过Browser.js SDK追加上传时，必须在Bucket跨域规则中暴露Headers：`x-oss-next-append-position`，否则无法正常获取追加位置`nextAppendPosition`，导致上传失败。更多信息，请参见[跨域设置](https://help.aliyun.com/zh/oss/user-guide/cors-settings)。
- 本文以OSS域名新建OSSClient为例。如果您希望通过自定义域名、STS等方式新建OSSClient，请参见[初始化（Browser.js SDK）](https://help.aliyun.com/zh/oss/developer-reference/initialization#concept-64095-zh)。
...
- 当文件已存在时，调用AppendObject接口会出现以下几种情况：如果文件为追加类型文件，且设置的追加位置和文件当前长度相等，则直接在该文件末尾追加内容。
...
## 相关文档
- 关于追加上传的完整代码示例，请参见[GitHub示例](https://github.com/ali-sdk/ali-oss#appendname-file-options)。
- 关于追加上传的API接口说明，请参见[AppendObject](https://help.aliyun.com/zh/oss/developer-reference/appendobject#reference-fvf-xld-5db)。

[上一篇：简单上传（Browser.js SDK）](/zh/oss/developer-reference/simple-upload-8)[下一篇：分片上传（Browser.js SDK）](/zh/oss/developer-reference/multipart-upload-11)该文章对您有帮助吗？反馈

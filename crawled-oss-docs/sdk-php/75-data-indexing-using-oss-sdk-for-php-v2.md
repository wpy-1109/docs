# 使用PHP SDK V2实现基于Object元数据的标量检索

Source: https://help.aliyun.com/zh/oss/developer-reference/data-indexing-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2实现基于Object元数据的标量检索-对象存储-阿里云
...
标量检索是OSS提供的基于Object元数据的索引功能，帮助您从Bucket存储的海量Object中快速查找与指定的Object名称、ETag、存储类型、大小、最后修改时间等条件匹配的Object。本文介绍如何使用PHP SDK V2进行标量检索。
...
以下代码用于为Bucket开启标量检索功能。开启后，OSS会为Bucket创建元数据索引库并为Bucket中的所有Object建立元数据索引。元数据索引库创建完成后，OSS会继续对Bucket中新增文件进行准实时的增量追踪扫描并为增量文件建立元数据索引。
...
## 相关文档
- 关于标量检索的更多操作指南，请参见[标量检索](https://help.aliyun.com/zh/oss/user-guide/scalar-retrieval/)。
- 关于数据索引的系列API接口，请参见[数据索引（Data Indexing）](https://help.aliyun.com/zh/oss/developer-reference/data-indexing/)。
- 关于标量检索的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/DoMetaQuery.php)。

[上一篇：数据索引（PHP SDK V2）](/zh/oss/developer-reference/data-index-using-oss-sdk-for-php-v2/)[下一篇：向量检索（PHP SDK V2）](/zh/oss/developer-reference/vector-retrieval-using-oss-sdk-for-php-v2)该文章对您有帮助吗？反馈

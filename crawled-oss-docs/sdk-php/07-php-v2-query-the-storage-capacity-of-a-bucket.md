# 使用PHP SDK V2获取存储空间统计信息

Source: https://help.aliyun.com/zh/oss/developer-reference/php-v2-query-the-storage-capacity-of-a-bucket

---

- 使用PHP SDK V2获取存储空间统计信息-对象存储-阿里云
...
// 创建获取存储空间统计信息的请求对象
$request = new Oss\Models\GetBucketStatRequest($bucket); // 创建请求对象，用于获取指定存储空间的统计信息。

// 调用getBucketStat方法获取存储空间统计信息
$result = $client->getBucketStat($request); // 调用getBucketStat方法获取存储空间的统计信息。

// 打印返回结果
printf(
    'status code:' . $result->statusCode . PHP_EOL . // HTTP响应状态码。
    'request id:' . $result->requestId . PHP_EOL . // 请求的唯一标识。
    'result:' . var_export($result, true) // 返回的结果，包含存储空间的统计信息。
);

```

## 相关文档
- 关于获取存储空间信息的完整示例代码，请参见[Github示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/GetBucketStat.php)。
- 关于获取存储空间信息的API接口，请参见[GetBucketStat](https://help.aliyun.com/zh/oss/developer-reference/getbucketstat#reference-2205923)。

[上一篇：获取存储空间的信息（PHP SDK V2）](/zh/oss/developer-reference/php-v2-query-bucket-information)[下一篇：删除存储空间（PHP SDK V2）](/zh/oss/developer-reference/php-v2-delete-a-bucket)该文章对您有帮助吗？反馈

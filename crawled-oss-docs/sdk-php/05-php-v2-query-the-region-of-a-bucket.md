# 使用PHP SDK V2获取存储空间地域

Source: https://help.aliyun.com/zh/oss/developer-reference/php-v2-query-the-region-of-a-bucket

---

- 使用PHP SDK V2获取存储空间地域-对象存储-阿里云
...
printf(
    ...
    'bucket location:' . var_export($result->location, true) // 存储空间所在的区域位置。
);

```

## 相关文档
- 关于获取存储空间地域的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-go-sdk-v2/blob/master/sample/get_bucket_location.go)。
- 关于获取存储空间地域的API接口，请参见[GetBucketLocation](https://help.aliyun.com/zh/oss/developer-reference/getbucketlocation#reference-e11-qtv-tdb)。

[上一篇：判断存储空间是否存在（PHP SDK V2）](/zh/oss/developer-reference/php-v2-determine-whether-a-bucket-exists)[下一篇：获取存储空间的信息（PHP SDK V2）](/zh/oss/developer-reference/php-v2-query-bucket-information)该文章对您有帮助吗？反馈

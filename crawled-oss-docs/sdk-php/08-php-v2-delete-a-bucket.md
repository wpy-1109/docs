...
Source: https://help.aliyun.com/zh/oss/developer-reference/php-v2-delete-a-bucket

---

- 使用PHP SDK V2删除存储空间-对象存储-阿里云
...
- 删除Bucket后，Bucket中的数据无法恢复，请在删除前确认数据已不再使用。如果需要继续使用Bucket中的数据，请提前做好备份。关于备份的更多信息，请参见[备份存储空间](https://help.aliyun.com/zh/oss/use-cases/back-up-buckets#concept-kzh-w4f-vdb)。
- 要删除存储空间，您必须有`oss:DeleteBucket`权限。具体操作，请参见[为RAM用户授予自定义的权限策略](https://help.aliyun.com/zh/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip)。
...
- 已具有`oss:DeleteBucket`权限。具体操作，请参见[为RAM用户授予自定义的权限策略](https://help.aliyun.com/zh/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip)。说明 如果您在RAM Policy中拥有`oss:DeleteBucket`权限但仍然无法删除Bucket，则Bucket Policy可能包含授权效力为Deny的`oss:DeleteBucket`权限。您必须将Deny修改为Allow或者直接删除此Bucket Policy，然后才能删除此Bucket。
...
相关文档
- 关于删除Bucket的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/DeleteBucket.php)。
- 关于删除Bucket的API接口说明，请参见[DeleteBucket](https://help.aliyun.com/zh/oss/developer-reference/deletebucket#reference-o1j-rrw-tdb)。

[上一篇：获取存储空间的存储容量（PHP SDK V2）](/zh/oss/developer-reference/php-v2-query-the-storage-capacity-of-a-bucket)[下一篇：存储空间标签（PHP SDK V2）](/zh/oss/developer-reference/php-v2-bucket-tagging)该文章对您有帮助吗？反馈

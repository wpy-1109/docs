...
Source: https://help.aliyun.com/zh/oss/developer-reference/multipart-upload-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2分片上传大文件-对象存储-阿里云
...
- 上传分片。  调用UploadPart方法上传分片数据。说明 对于同一个uploadID，分片号（partNumber）标识了该分片在整个文件内的相对位置。如果使用同一个分片号上传了新的数据，那么OSS上该分片已有的数据将会被覆盖。
...
如果您希望在文件分片上传后通知应用服务器，可参考以下代码示例。
...
## 相关文档
- 关于分片上传的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/CompleteMultipartUpload.php)。

[上一篇：追加上传（PHP SDK V2）](/zh/oss/developer-reference/append-upload-using-oss-sdk-for-php-v2)[下一篇：表单上传（PHP SDK V2）](/zh/oss/developer-reference/form-upload-using-oss-sdk-for-php-v2)该文章对您有帮助吗？反馈

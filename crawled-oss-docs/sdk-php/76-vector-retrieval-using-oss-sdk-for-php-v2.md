# 使用PHP SDK V2通过向量检索快速定位Object

Source: https://help.aliyun.com/zh/oss/developer-reference/vector-retrieval-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2通过向量检索快速定位Object-对象存储-阿里云
...
通过OSS向量检索，您可以基于语义内容、OSS元数据、多媒体元数据、对象ETag及标签和自定义元数据等条件，快速定位海量Object中的目标文件，优化检索效率。本文介绍如何使用PHP SDK V2进行向量检索。
...
- 本文以从环境变量读取访问凭证为例。更多配置访问凭证的示例，请参见[配置访问凭证](https://help.aliyun.com/zh/oss/developer-reference/configure-access-credentials-for-php-v2)。
- 使用PHP SDK发起OSS请求，您需要初始化一个OSSClient。本文通过默认配置示例来创建一个OSSClient。更多配置客户端的示例，请参见[配置客户端](https://help.aliyun.com/zh/oss/developer-reference/configure-client-using-oss-sdk-for-php-v2)。
...
// 创建GetMetaQueryStatusRequest对象，用于获取Bucket的元数据查询状态
...
printf(
    ...
    'meta query status:' . var_export($result->metaQueryStatus, true) . PHP_EOL // 元数据查询功能的状态（如开启、关闭等）
);
...
以下代码通过向量检索功能查询满足指定条件Object，并按照指定字段和排序方式列出Object信息。
...
## 相关文档
- 关于向量检索的更多操作指南，请参见[向量检索](https://help.aliyun.com/zh/oss/user-guide/vector-retrieval/)。
- 关于数据索引的系列API接口，请参见[数据索引（Data Indexing）](https://help.aliyun.com/zh/oss/developer-reference/data-indexing/)。
- 关于向量检索的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/DoMetaQuery.php)。

[上一篇：标量检索（PHP SDK V2）](/zh/oss/developer-reference/data-indexing-using-oss-sdk-for-php-v2)[下一篇：传输管理（PHP SDK V2）](/zh/oss/developer-reference/transfer-management-using-oss-sdk-for-php-v2/)该文章对您有帮助吗？反馈

# 使用PHPSDKV2配置存储空间访问跟踪

Source: https://help.aliyun.com/zh/oss/developer-reference/php-access-tracking

---

- 使用PHPSDKV2配置存储空间访问跟踪-对象存储-阿里云
...
本文介绍如何使用PHP SDK V2配置存储空间（Bucket）的访问跟踪功能。
...
$accessMonitorConfiguration = new Oss\Models\AccessMonitorConfiguration(
    status: Oss\Models\AccessMonitorStatusType::ENABLED // 启用访问监控
);

// 创建设置存储空间访问监控的请求对象 并将配置内容传入
$request = new Oss\Models\PutBucketAccessMonitorRequest(
    bucket: $bucket,
    accessMonitorConfiguration: $accessMonitorConfiguration
);

// 调用putBucketAccessMonitor方法设置存储空间的访问监控
...
// 调用getBucketAccessMonitor方法获取存储空间的访问监控状态
...
printf(
    ...
    'access monitor status:' . $result->accessMonitorConfiguration->status // 访问监控状态
);

```

## 相关文档
- 关于开启访问跟踪的API接口说明，请参见[PutBucketAccessMonitor](https://help.aliyun.com/zh/oss/developer-reference/putbucketaccessmonitor#concept-2114625) 。
- 关于查看访问跟踪状态的API接口说明，请参见[GetBucketAccessMonitor](https://help.aliyun.com/zh/oss/developer-reference/getbucketaccessmonitor#concept-2114879)。

[上一篇：数据复制（PHP SDK V2）](/zh/oss/developer-reference/data-replication-using-oss-sdk-for-php-v2)[下一篇：生命周期管理（PHP SDK V2）](/zh/oss/developer-reference/php-lifecycle-management)该文章对您有帮助吗？反馈

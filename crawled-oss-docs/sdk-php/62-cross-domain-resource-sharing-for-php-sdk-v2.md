# 使用PHP SDK V2管理跨域资源共享规则

Source: https://help.aliyun.com/zh/oss/developer-reference/cross-domain-resource-sharing-for-php-sdk-v2

---

- 使用PHP SDK V2管理跨域资源共享规则-对象存储-阿里云
...
- 要设置跨域规则，您必须有`oss:PutBucketCors`权限；要获取跨域规则，您必须有`oss:GetBucketCors`权限；要删除跨域规则，您必须有`oss:DeleteBucketCors`权限。具体操作，请参见[为RAM用户授予自定义的权限策略](https://help.aliyun.com/zh/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip)。
...
// 创建设置存储空间跨域资源共享CORS配置的请求对象 配置允许所有来源和指定方法
$request = new Oss\Models\PutBucketCorsRequest(bucket: $bucket, corsConfiguration: new Oss\Models\CORSConfiguration(
        array(
            new Oss\Models\CORSRule(
                ...
                allowedMethods: ['PUT', 'GET'] // 允许的方法
            )
        )
    )
);

// 调用putBucketCors方法设置存储空间的CORS配置
...
printf(
    ...
    'cors:' . var_export($result->corsConfiguration, true) // CORS配置内容
);
...
## 相关文档
- 关于跨域资源共享的完整示例代码，请参见[PutBucketCors](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/PutBucketCors.php)，[GetBucketCors](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/GetBucketCors.php)和[DeleteBucketCors](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/DeleteBucketCors.php)。
...
[上一篇：防盗链（PHP SDK V2）](/zh/oss/developer-reference/php-v2-hotlink-protection)[下一篇：合规保留策略（PHP SDK V2）](/zh/oss/developer-reference/php-v2-compliance-reserved-resources)该文章对您有帮助吗？反馈

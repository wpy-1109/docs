...
Source: https://help.aliyun.com/zh/oss/developer-reference/php-v2-bucket-tagging

---

- 使用PHP SDK管理存储空间标签-对象存储-阿里云
...
// 创建获取存储空间标签的请求对象
$request = new Oss\Models\GetBucketTagsRequest(bucket: $bucket); // 创建请求对象，用于获取指定存储空间的标签信息

// 调用getBucketTags方法获取存储空间标签
$result = $client->getBucketTags($request); // 调用getBucketTags方法获取存储空间的标签信息

// 打印返回结果
printf(
    'status code:' . $result->statusCode . PHP_EOL . // HTTP响应状态码
    'request id:' . $result->requestId . PHP_EOL . // 请求的唯一标识
    'tags:' . var_export($result->tagging, true) // 返回的标签信息，以数组形式展示
);
...
## 相关文档
- 关于设置Bucket标签的API接口说明，请参见[PutBucketTags](https://help.aliyun.com/zh/oss/developer-reference/putbuckettags#concept-261782)。
- 关于获取Bucket标签的API接口说明，请参见[GetBucketTags](https://help.aliyun.com/zh/oss/developer-reference/getbuckettags#concept-261796)。
- 关于删除Bucket标签的API接口说明，请参见[DeleteBucketTags](https://help.aliyun.com/zh/oss/developer-reference/deletebuckettags#concept-261821)。

[上一篇：删除存储空间（PHP SDK V2）](/zh/oss/developer-reference/php-v2-delete-a-bucket)[下一篇：请求者付费模式（PHP SDK V2）](/zh/oss/developer-reference/php-v2-pay-by-requester)该文章对您有帮助吗？反馈

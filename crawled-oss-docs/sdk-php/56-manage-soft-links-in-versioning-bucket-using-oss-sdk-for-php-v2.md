# 使用PHP SDK V2创建与获取软链接

Source: https://help.aliyun.com/zh/oss/developer-reference/manage-soft-links-in-versioning-bucket-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2创建与获取软链接-对象存储-阿里云
...
本文介绍如何使用OSS PHP SDK在受版本控制的存储空间（Bucket）中创建软链接和获取软链接。
...
### 创建软链接
说明 - 软链接本身也可以有多个版本，每个不同的版本可以指向不同的TargetObject，版本ID由OSS自动生成，在响应header中返回x-oss-version-id。
...
[上一篇：管理文件访问权限（PHP SDK V2）](/zh/oss/developer-reference/manage-object-acls-in-versioning-bucket-using-oss-sdk-for-php-v2)[下一篇：对象标签（PHP SDK V2）](/zh/oss/developer-reference/versioning-object-tagging-using-oss-sdk-for-php-v2/)该文章对您有帮助吗？反馈

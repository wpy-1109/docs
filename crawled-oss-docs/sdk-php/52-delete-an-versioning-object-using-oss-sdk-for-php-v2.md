# 使用PHP SDK V2删除版本控制下的文件

Source: https://help.aliyun.com/zh/oss/developer-reference/delete-an-versioning-object-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2删除版本控制下的文件-对象存储-阿里云
...
对于开启了版本控制的Bucket，OSS支持根据是否传入versionId控制对象（Object）被临时删除或是彻底删除。您可以结合这个特性对单个、多个或者指定前缀的Object进行删除操作。
...
版本控制下的删除行为说明如下：
- 未指定versionId（临时删除）： 如果在未指定versionId的情况下执行删除操作时，默认不会删除Object的当前版本，而是对当前版本插入删除标记（Delete Marker）。当执行GetObject操作时，OSS会检测到当前版本为删除标记，并返回`404 Not Found`。此外，响应中会返回`header：x-oss-delete-marker = true`以及新生成的删除标记的版本号`x-oss-version-id`。`x-oss-delete-marker`的值为true，表示与返回的`x-oss-version-id`对应的版本为删除标记。关于临时删除文件后如何恢复文件的更多信息，请参见[恢复文件](https://help.aliyun.com/zh/oss/user-guide/manage-objects-in-a-versioning-enabled-bucket#section-y71-fc5-oe7)。
- 指定versionId（彻底删除）： 如果在指定versionId的情况下执行删除操作时，OSS会根据`params`中指定的`versionId`参数彻底删除该版本。如果要删除ID为“null”的版本，请在`params`参数中添加`params['versionId'] = “null”`。

## 示例代码

#### 指定versionId删除Object

以下代码用于指定versionId对Object进行彻底删除：
...
[上一篇：拷贝文件（PHP SDK V2）](/zh/oss/developer-reference/copy-an-versioning-object-using-oss-sdk-for-php-v2)[下一篇：解冻文件](/zh/oss/developer-reference/unfreeze-file-1)该文章对您有帮助吗？反馈

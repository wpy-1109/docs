# 使用PHP SDK V2解冻归档存储文件

Source: https://help.aliyun.com/zh/oss/developer-reference/restore-object-using-oss-sdk-for-php-v2

---

- 使用PHP SDK V2解冻归档存储文件-对象存储-阿里云
...
归档、冷归档以及深度冷归档类型的文件需要解冻之后才能读取。文件解冻后，会产生一份文件副本，即同时存在文件副本和归档、冷归档或深度冷归档的原文件。在文件解冻的有效期过后，文件副本会自动删除。本文介绍如何使用OSS Go SDK解冻归档、冷归档以及深度冷归档文件。

## 注意事项
- 仅归档、冷归档以及深度冷归档类型的Object支持调用RestoreObject方法。
...
- 要解冻文件，您必须有`oss:RestoreObject`权限。具体操作，请参见[为RAM用户授予自定义的权限策略](https://help.aliyun.com/zh/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip)。
...
冷归档或者深度冷归档存储类型文件（Object）在数据解冻时会生成一份文件副本用于访问，文件副本在解冻时间结束前会收取临时存储费用。如果您希望提前结束解冻状态，且文件副本不再产生临时存储费用，您可以发起CleanRestoredObject请求。请求完成后，Object将恢复为冷冻状态，不能直接读取。
...
// 创建CleanRestoredObjectRequest对象，用于清理已恢复的归档存储类型对象
...
## 相关文档
- 关于解冻文件的完整示例代码，请参见[GitHub示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/RestoreObject.php)。
- 关于解冻文件的API接口，请参见[RestoreObject](https://pkg.go.dev/github.com/aliyun/alibabacloud-oss-go-sdk-v2/oss#Client.RestoreObject)。
- 关于解冻文件的更多操作信息，请参见[解冻文件](https://help.aliyun.com/zh/oss/user-guide/restore-objects-for-access)。

[上一篇：删除文件（PHP SDK V2）](/zh/oss/developer-reference/delete-objects-using-oss-sdk-for-php-v2)[下一篇：管理文件元数据（PHP SDK V2）](/zh/oss/developer-reference/manage-object-metadata-using-oss-sdk-for-php-v2)该文章对您有帮助吗？反馈

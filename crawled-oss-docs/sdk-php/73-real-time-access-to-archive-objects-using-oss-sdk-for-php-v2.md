# 使用PHPSDKV2管理Bucket的归档直读

Source: https://help.aliyun.com/zh/oss/developer-reference/real-time-access-to-archive-objects-using-oss-sdk-for-php-v2

---

- 使用PHPSDKV2管理Bucket的归档直读-对象存储-阿里云
...
本文介绍如何使用PHP SDK V2为Bucket开启或关闭归档直读的功能。
...
$request = new Oss\Models\PutBucketArchiveDirectReadRequest(
    ...
    archiveDirectReadConfiguration: new Oss\Models\ArchiveDirectReadConfiguration(
        enabled: true // 启用归档直读功能
    )
);

// 调用putBucketArchiveDirectRead方法设置存储空间的归档直读功能
...
// 创建获取存储空间归档直读功能配置的请求对象
$request = new Oss\Models\GetBucketArchiveDirectReadRequest(
    bucket: $bucket // 存储空间名称
);

// 调用getBucketArchiveDirectRead方法获取存储空间的归档直读功能配置
...
printf(
    ...
    'archive direct read config:' . var_export($result->archiveDirectReadConfiguration->enabled, true) . PHP_EOL // 归档直读功能是否启用的状态
);

```

## 相关文档
- 关于开启Bucket归档直读的完整示例，请参见[Github示例](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/blob/master/sample/PutBucketArchiveDirectRead.php)。
- 关于开启Bucket归档直读的API接口说明，请参见[PutBucketArchiveDirectRead](https://help.aliyun.com/zh/oss/developer-reference/putbucketarchivedirectread#DAS)。
- 关于查看Bucket是否开启归档直读的API接口说明，请参见[GetBucketArchiveDirectRead](https://help.aliyun.com/zh/oss/developer-reference/getbucketarchivedirectread#DAS)。

[上一篇：日志转存（PHP SDK V2）](/zh/oss/developer-reference/php-logging-2)[下一篇：数据索引（PHP SDK V2）](/zh/oss/developer-reference/data-index-using-oss-sdk-for-php-v2/)该文章对您有帮助吗？反馈

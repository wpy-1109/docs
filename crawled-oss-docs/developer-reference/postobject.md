...
To upload a file using an HTML form, you must have the `oss:PutObject` permission. For more information, see [Attach a custom policy to a RAM user](https://www.alibabacloud.com/help/en/oss/user-guide/common-examples-of-ram-policies#section-ucu-jv0-zip).
...
| --- | --- | --- | --- |
| Content-Type | String | No | The type of the file and the encoding of the web page. This determines how browsers read and encode the file.The form submitted in a Post operation must be encoded in the multipart/form-data format. This means the Content-Type header is in the multipart/form-data;boundary=xxxxxx format.The boundary is a random string generated by the form. You do not need to specify it. If you use an SDK to construct the form, the SDK also generates a random value. |
...
The following table describes the form elements that are common to both V1 and V4 signatures. For information about form elements that are unique to V4 signatures, see [V4 signature form](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-4-recommend#79752ac45behk). For information about form elements that are unique to V1 signatures, see [V1 signature form](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-1#79752ac45behk).
...
(https://www.alibabacloud.com/help/en/ram/api-assumerole#reference-clc-3sv-xdb)
...
| key | String | Yes | The name of the object to upload. Do not encode the name. If the name includes a path, such as destfolder/example.jpg, OSS automatically creates the corresponding folder. Default value: none. |
...
| success_action_status | String | No | When the success_action_redirect form field is not specified, this form field specifies the status code that is returned to the client after a successful upload. Valid values: 200, 201, and 204 (default).If this field is set to 200 or 204, OSS returns an empty document and the corresponding status code. If this field is set to 201, OSS returns an XML file and the 201 status code. If this field is not set or is set to an invalid value, OSS returns an empty document and the 204 status code. |
| x-oss-meta-* | String | No | The user-specified user meta. Default value: none.If the request includes a form field that is prefixed with x-oss-meta-, the field is considered user meta. For example, x-oss-meta-location. Note An object can have multiple such parameters, but the total size of all user meta cannot exceed 8 KB. |
...
This operation also returns common response headers, such as Date and x-oss-request-id. For more information, see [Common response headers](https://www.alibabacloud.com/help/en/oss/developer-reference/common-http-headers#section-hcd-m2y-wdb).
...
[Java](https://www.alibabacloud.com/help/en/oss/developer-reference/form-upload-1)

-

[Go V2](https://www.alibabacloud.com/help/en/oss/developer-reference/v2-postobject)
...
For more information about the policy for a V4 signature in a POST request, see [POST request V4 signature policy](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-4-recommend#49c0713824yc9)

-

For more information about the policy for a V1 signature in a POST request, see [POST request V1 signature policy](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-1#49c0713824yc9).
...
For more information about V4 signatures in POST requests, see [POST request V4 signature](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-4-recommend).

-

For more information about V1 signatures in POST requests, see [POST request V1 signature](https://www.alibabacloud.com/help/en/oss/developer-reference/signature-version-1).
...
For an example of how to directly upload data to OSS from a web client using a form, see [JavaScript client-side signature-based direct upload](https://www.alibabacloud.com/help/en/oss/use-cases/add-signatures-on-the-client-by-using-javascript-and-upload-data-to-oss#concept-frd-4gy-5db).

-

For information about common errors that may occur when you call the PostObject operation and their solutions, see [Common errors of PostObject and troubleshooting](https://www.alibabacloud.com/help/en/oss/how-to-handle-common-errors-when-the-postobject-operation-is-called#concept-nrx-jfj-wdb).
